...
from data_utils import initialize_data # MODIFIED: Import initialize_data instead of EpisodicHDF5DatasetRAM
...
def main():
    ...
    print(f"Train DataLoader: {len(train_dataloader.dataset)} samples, Val DataLoader: {len(val_dataloader.dataset)} samples")
    ...
    if len(train_dataloader) > 0:
        ...
        try:
            ...
            current_obs_batch = {
                key: val[:, 0] if key == POLICY_QPOS_KEY and val.ndim > 2 else val # Select first qpos if it's a sequence
            ...
            print(f"  Error during policy select_action (Eval Mode with train_batch): {e}")
            import traceback
            traceback.print_exc()
    else:
