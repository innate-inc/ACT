...
from data_utils import (
    initialize_data,
)  # Assuming data_utils.py is in the same directory or PYTHONPATH

# --- Configuration ---
# Data parameters
DATA_DIR = (
    "/home/vignesh/raid/PaperCorner_Filtered"  # Replace with your actual data directory
)
CHUNK_SIZE = 30
TRAIN_VAL_SPLIT = 0.9
...
TASK_NAME = os.path.basename(
    DATA_DIR.rstrip("/")
)  # Extract directory name from DATA_DIR
...
def main():
    ...
    if not os.getenv("WANDB_API_KEY"):  # Double check if it's set now
        print(
            "Warning: WANDB_API_KEY is still not set. WandB logging will likely fail."
        )
    ...
    try:
        ...
        print(
            f"Please ensure your data directory '{DATA_DIR}' is correctly set and contains 'metadata.json' and episode files."
        )
    ...
    if val_dataloader and len(val_dataloader) > 0:
        ...
        print(
            "No validation data or validation split resulted in 0 validation episodes."
        )
    ...
    optimizer = optim.AdamW(
        policy.get_optim_params(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY
    )
    ...
    overall_pbar = tqdm(
        range(NUM_EPOCHS), unit="epoch", desc="Overall Training Progress"
    )
    ...
    for epoch in overall_pbar:  # `epoch` is the current epoch index (0, 1, ...)
        ...
        for batch_idx, batch in enumerate(
            train_dataloader
        ):  # Iterate directly, no inner tqdm
            ...
            for key, tensor in batch.items():
                if isinstance(tensor, torch.Tensor):
                    ...
                    batch_device[key] = (
                        tensor  # e.g. 'action_is_pad' could be bool list
                    )
        ...
        if len(train_dataloader) > 0:
            ...
            avg_kld_loss = (
                epoch_kld_loss_sum / len(train_dataloader) if USE_VAE else 0.0
            )
            ...
            overall_pbar.write(
                "Warning: train_dataloader is empty. Cannot compute epoch averages."
            )
        ...
        if val_dataloader and len(val_dataloader) > 0 and (epoch + 1) % 5 == 0:
            ...
            latest_val_avg_kld_loss = (
                f"{(val_kld_loss_sum / len(val_dataloader) if USE_VAE else 0):.4f}"
            )

            wandb.log(
                {
                    "val/avg_total_loss": (
                        float(latest_val_avg_loss)
                        if latest_val_avg_loss != "N/A"
                        else 0
                    ),
                    "val/avg_l1_loss": (
                        float(latest_val_avg_l1_loss)
                        if latest_val_avg_l1_loss != "N/A"
                        else 0
                    ),
                    "val/avg_kld_loss": (
                        float(latest_val_avg_kld_loss)
                        if latest_val_avg_kld_loss != "N/A"
                        else 0
                    ),
                    "epoch": epoch,
                }
