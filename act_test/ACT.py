...
def create_stats_buffers(
    ...
    for key, ft in features.items():
        ...
        if norm_mode == NormalizationMode.MEAN_STD:
            ...
            max_val = (
                torch.ones(shape, dtype=torch.float32) * torch.inf
            )  # Renamed to avoid conflict with builtin max
            buffer_dict = {
        ...
        if stats and key in stats:
            ...
            if norm_mode == NormalizationMode.MEAN_STD:
                if "mean" in stats[key]:
                    buffer["mean"].data = (
                        stats[key]["mean"].clone().to(dtype=torch.float32).view(shape)
                    )
                if "std" in stats[key]:
                    buffer["std"].data = (
                        stats[key]["std"].clone().to(dtype=torch.float32).view(shape)
                    )
            elif norm_mode == NormalizationMode.MIN_MAX:
                if "min" in stats[key]:
                    buffer["min"].data = (
                        stats[key]["min"].clone().to(dtype=torch.float32).view(shape)
                    )
                if "max" in stats[key]:
                    buffer["max"].data = (
                        stats[key]["max"].clone().to(dtype=torch.float32).view(shape)
                    )
...
class Normalize(nn.Module):
    def __init__(
        ...
        ],  # Dict[FeatureType_str, NormalizationMode_str]
        stats: Optional[Dict[str, Dict[str, Tensor]]] = None,
    ):
        super().__init__()
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        for key, ft in self.features.items():
            ...
            norm_mode_str = self.norm_map.get(
                ft.type.value, NormalizationMode.IDENTITY.value
            )
            norm_mode = NormalizationMode(norm_mode_str)
            ...
            if not hasattr(self, buffer_name):
                ...
                print(
                    f"Warning: No normalization buffer found for {key}. Skipping normalization."
                )
                continue
            ...
            if norm_mode == NormalizationMode.MEAN_STD:
                ...
                normalized_to_0_1 = (processed_batch[key] - min_val) / (
...
class Unnormalize(nn.Module):
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        for key, ft in self.features.items():
            ...
            if not hasattr(self, buffer_name):
                print(
                    f"Warning: No normalization buffer found for {key}. Skipping unnormalization."
                )
                continue
            ...
            if norm_mode == NormalizationMode.MEAN_STD:
                ...
                processed_batch[key] = (
                    unnormalized_from_0_1 * (max_val - min_val) + min_val
                )
            else:
                raise ValueError(f"Unsupported normalization mode: {norm_mode}")
        return processed_batch
...
class ACTConfig:
    ...
    def image_input_keys(self) -> List[str]:
        ...
        return sorted(
            [k for k in self.input_shapes if k.startswith("observation.image")]
        )

    def _get_policy_features(
        self, shapes_dict: Dict[str, List[int]]
...
class ACTPolicy(nn.Module):
    ...
    def _prepare_batch_for_model(self, batch: Dict[str, Tensor]) -> Dict[str, Tensor]:
        ...
        if self.config.image_input_keys:
            ...
            if images_to_collate:
                ...
                "observation.images" not in processed_batch
                and self.config.image_input_keys
            ):
    ...
    def select_action(self, batch: Dict[str, Tensor]) -> Tensor:
        ...
        working_batch = {
            k: v.clone() if isinstance(v, Tensor) else v for k, v in batch.items()
        }
        ...
        if self.config.temporal_ensemble_coeff is not None:
            ...
            actions_chunk = self.unnormalize_outputs(
                {"action": actions_chunk_normalized}
            )["action"]
        ...
        if len(self._action_queue) == 0:
            actions_chunk_normalized = self.model(model_batch)[0]
            actions_to_queue_normalized = actions_chunk_normalized[
                :, : self.config.n_action_steps
            ]
            actions_to_queue = self.unnormalize_outputs(
                {"action": actions_to_queue_normalized}
            )["action"]
    ...
    def forward(self, batch: Dict[str, Tensor]) -> Tuple[Tensor, Dict]:
        ...
        if (
            "action_is_pad" in working_batch
        ):  # Use padding info from the (potentially modified) working_batch
        ...
        if (
            self.config.use_vae and mu_hat is not None and log_sigma_x2_hat is not None
        ):  # Check VAE was active
            # KL divergence
            mean_kld = (
...
class ACTDecoderLayer(nn.Module):
    ...
    def maybe_add_pos_embed(
        self, tensor: Tensor, pos_embed: Optional[Tensor]
    ) -> Tensor:
...
class ACT(nn.Module):
    ...
    def __init__(self, config: ACTConfig):
        ...
        if self.config.use_vae:
            ...
            self.register_buffer(
                ...
                create_sinusoidal_pos_embedding(
                    num_input_token_vae_encoder, config.dim_model
                ).unsqueeze(0),
            )
        ...
        if "observation.environment_state" in self.config.input_shapes:
            self.encoder_env_state_input_proj = nn.Linear(
                self.config.input_shapes["observation.environment_state"][0],
                config.dim_model,
            )
        ...
        self.action_head = nn.Linear(
            config.dim_model, self.config.output_shapes["action"][0]
        )
    ...
    def forward(
        ...
        if self.config.use_vae and self.training and "action" in batch:
            cls_embed = self.vae_encoder_cls_embed.weight.unsqueeze(0).repeat(
                batch_size, 1, 1
            )

            vae_tokens = [cls_embed]
            if "observation.state" in batch and hasattr(
            ...
            if "action_is_pad" in batch:
                num_prefix_tokens = (
                    vae_encoder_input.shape[1] - batch["action_is_pad"].shape[1]
                )
                ...
                key_padding_mask_vae = torch.cat(
                    [prefix_pad, batch["action_is_pad"]], dim=1
                )
            ...
            log_sigma_x2 = latent_pdf_params[
            ...
            latent_sample = torch.zeros(
                batch_size, self.config.latent_dim, device=device
            )
        ...
        if "observation.environment_state" in batch and hasattr(
        ...
        if (
            ...
            for i, img_tensor in enumerate(
                ...
                projected_feature = self.encoder_img_feat_input_proj(
                    feature_map
                )  # (B, D_model, H', W')
                ...
                pos_embed_template_2d = self.encoder_cam_feat_pos_embed(
                    projected_feature[:1]
                ).to(
                ...
                rearranged_projected_feature = einops.rearrange(
                    projected_feature, "b c h w -> (h w) b c"
                )
                rearranged_pos_embed = einops.rearrange(
                    pos_embed_2d_batched, "b c h w -> (h w) b c"
                )
        ...
        if (
            all_cam_features_processed
        ):  # Check if there are camera features to concatenate
            full_encoder_input_sequence = torch.cat(
                [encoder_1d_sequence] + all_cam_features_processed, dim=0
            )
            full_encoder_pos_embed = torch.cat(
                [encoder_1d_pos_embed_sequence] + all_cam_pos_embeds_processed, dim=0
            )
        ...
        encoder_output = self.encoder(
            full_encoder_input_sequence, pos_embed=full_encoder_pos_embed
        )  # (S_total, B, D)
        ...
        decoder_query_embed = self.decoder_pos_embed.weight.unsqueeze(1).repeat(
            1, batch_size, 1
        )  # (chunk_size, B, D)
