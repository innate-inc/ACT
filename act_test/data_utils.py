...
class EpisodicHDF5DatasetRAM(Dataset):
    ...
    def __init__(
        ...
        if self.use_img_aug and self.transforms is None:
            self.transforms = v2.Compose(
                [v2.RandomPhotometricDistort(p=0.5), v2.RandomHorizontalFlip(p=0.5)]
            )
        ...
        for ep_id in episode_ids:
            ...
            with h5py.File(file_path, "r") as f:
                ...
                ep_data_ram = {}
                ep_data_ram[self.POLICY_ACTION_KEY] = torch.from_numpy(
                    f[self.H5_ACTION_KEY][:]
                ).float()
                ep_data_ram[self.POLICY_QPOS_KEY] = torch.from_numpy(
                    f[self.H5_QPOS_KEY][:]
                ).float()
                ...
                for i, h5_cam_name in enumerate(self.H5_CAMERA_NAMES):
                    ...
                    img_tchw_float = (
                        torch.from_numpy(img_thwc_uint8).float().permute(0, 3, 1, 2)
                        / 255.0
                    )
        ...
        if not self.loaded_episodes_data:  # Handle case with no episodes
            if self.POLICY_QPOS_KEY:
                stats[self.POLICY_QPOS_KEY] = {
                    "mean": torch.zeros(self.QPOS_DIM),
                    "std": torch.ones(self.QPOS_DIM),
                }
            ...
            for key in self.POLICY_IMG_KEYS:
                stats[key] = {
                    "mean": torch.zeros(self.IMAGE_C, 1, 1),
                    "std": torch.ones(self.IMAGE_C, 1, 1),
                }
            return stats

        # Collect tensor references for stats from self.loaded_episodes_data
        qpos_tensors_for_stats = [
            ep_data[self.POLICY_QPOS_KEY]
            for ep_data in self.loaded_episodes_data
            if self.POLICY_QPOS_KEY in ep_data
        ]
        action_tensors_for_stats = [
            ep_data[self.POLICY_ACTION_KEY]
            for ep_data in self.loaded_episodes_data
            if self.POLICY_ACTION_KEY in ep_data
        ]

        img_tensors_for_stats: Dict[str, List[torch.Tensor]] = {
            key: [] for key in self.POLICY_IMG_KEYS
        }
        for key in self.POLICY_IMG_KEYS:
            for ep_data in self.loaded_episodes_data:
                if key in ep_data:
        ...
        for policy_img_key in self.POLICY_IMG_KEYS:
            if img_tensors_for_stats[policy_img_key]:
                ...
                stats[policy_img_key] = {
                    ...
                    "std": torch.clamp(
                        torch.std(img_tensor_all, dim=(0, 2, 3)).reshape(-1, 1, 1),
                        min=1e-6,
                    ),
                }
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        padded_actions = torch.zeros(
            (self.chunk_size, self.ACTION_DIM), dtype=torch.float32
        )
...
def initialize_data(
    ...
    all_episode_ids = [
        ep_info["episode_id"] for ep_info in metadata.get("episodes", [])
    ]
    if not all_episode_ids:
        raise ValueError(
            "No episodes found in metadata.json or 'episodes' key is missing/empty."
        )

    print(
        f"Found a total of {len(all_episode_ids)} episodes: {sorted(all_episode_ids)}"
    )
    ...
    print(
        f"Using {len(train_episode_ids)} episodes for training: {sorted(train_episode_ids)}"
    )
    print(
        f"Using {len(val_episode_ids)} episodes for validation: {sorted(val_episode_ids)}"
    )
