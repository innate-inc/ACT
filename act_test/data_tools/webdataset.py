...
def convert_episode_to_samples(hdf5_path, episode_id):
    """
    ...
    try:
        with h5py.File(hdf5_path, 'r') as f:
            ...
            for timestep in range(num_timesteps):
                ...
                # Convert camera images to PNG bytes using cv2
                cam1_img = camera1_images[timestep].astype(np.uint8)
                # Convert RGB to BGR for cv2 (cv2 expects BGR format)
                cam1_img_bgr = cv2.cvtColor(cam1_img, cv2.COLOR_RGB2BGR)
                _, cam1_encoded = cv2.imencode('.png', cam1_img_bgr)
                cam1_bytes = cam1_encoded.tobytes()
                
                cam2_img = camera2_images[timestep].astype(np.uint8)
                # Convert RGB to BGR for cv2 (cv2 expects BGR format)
                cam2_img_bgr = cv2.cvtColor(cam2_img, cv2.COLOR_RGB2BGR)
                _, cam2_encoded = cv2.imencode('.png', cam2_img_bgr)
                cam2_bytes = cam2_encoded.tobytes()
                
                # Convert qpos to numpy bytes
                qpos_buffer = io.BytesIO()
                np.save(qpos_buffer, qpos[timestep].astype(np.float16))
                ...
                np.save(actions_buffer, actions_future.astype(np.float16))
...
def convert_hdf5_to_webdataset(hdf5_directory, webd_directory, shard_size=1000):
    """
    ...
    try:
        ...
        with tqdm(total=total_timesteps, desc="🔄 Converting timesteps", unit="samples") as pbar:
            for episode_info, episode_id, hdf5_path, expected_timesteps in valid_episodes:
                ...
                episode_samples, actual_timesteps = convert_episode_to_samples(hdf5_path, episode_id)
        ...
        dataset_info = {
            ...
            'sample_format': {
                'cam1.png': 'RGB image from camera 1',
                'cam2.png': 'RGB image from camera 2', 
                'qpos.npy': 'Joint positions (float16)',
                'actions.npy': 'Future actions from current timestep (float16)'
            }
        }
