...
from data_tools.webdataset import (
...
def convert_data_always(data_dir):
    ...
    success = convert_hdf5_to_webdataset(
        hdf5_directory=DATA_DIR, webd_directory=WEBD_DIR, shard_size=SHARD_SIZE
    )
...
def train_ddp(rank, world_size, args, webd_dir):
    ...
    try:
        ...
        if rank == 0:
            ...
            print(
                f"Please ensure your data directory '{WEBD_DIR}' contains WebDataset .tar files."
            )
    ...
    if rank == 0:
        print(
            "WebDataset dataloaders initialized (streaming datasets - no fixed length)"
        )
    ...
    # Wrap model with DDP
    policy = DDP(
        policy, device_ids=[rank], output_device=rank, find_unused_parameters=False
    )

    optimizer = optim.AdamW(
        policy.module.get_optim_params(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY
    )
    ...
    while step < MAX_STEPS:
        ...
        if rank == 0:
            ...
            if step % 100 == 0:
                avg_batch_load = (
                    sum(batch_load_times[-100:]) / len(batch_load_times[-100:]) * 1000
                )
                avg_forward = (
                    sum(forward_times[-100:]) / len(forward_times[-100:]) * 1000
                )
                avg_backward = (
                    sum(backward_times[-100:]) / len(backward_times[-100:]) * 1000
                )
                avg_optimizer = (
                    sum(optimizer_times[-100:]) / len(optimizer_times[-100:]) * 1000
                )
                avg_total = (
                    sum(total_step_times[-100:]) / len(total_step_times[-100:]) * 1000
                )
            ...
            if rank == 0:
                ...
                with torch.no_grad():
                    try:
                        ...
                        overall_pbar.write(
                            f"Validation at step {step}: Loss={loss.item():.4f}, "
                        ...
                        val_iter = iter(val_dataloader)
                        overall_pbar.write(
                            f"Validation dataset exhausted at step {step}, resetting iterator"
                        )
                    except Exception as e:
...
def main():
    ...
    parser.add_argument(
        "--chunk_size", type=int, default=30, help="Action sequence length / chunk size"
    )
    parser.add_argument(
        "--max_steps", type=int, default=15000, help="Maximum number of training steps"
    )
    parser.add_argument(
        "--learning_rate", type=float, default=5e-5, help="Main learning rate"
    )
    parser.add_argument(
